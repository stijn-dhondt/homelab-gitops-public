...
| `grafana-dashboard-cilium-agent-configmap.yaml` | Cilium's own data-plane/control-plane dashboard ([grafana.com #21431](https://grafana.com/grafana/dashboards/21431-cilium-metrics/), last sourced from the `cilium/cilium` repo's `v1.19` branch — the chart has since moved to `1.20.0` (see `helmrelease.yaml`) and this dashboard hasn't been re-synced against the `v1.20` branch yet). |
| `grafana-dashboard-hubble-configmap.yaml` | Hubble flow-observability dashboard (same repo/branch as above, `hubble-dashboard.json`) — complements the live Hubble UI with historical trends. |
