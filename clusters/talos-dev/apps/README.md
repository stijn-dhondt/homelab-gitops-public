...
| [Forgejo](forgejo/README.md) | Self-hosted Git server, at `forgeo.lab.example.com` — deliberately not behind Authentik (would break `git clone`/`push` over HTTPS), keeps its own built-in auth |
